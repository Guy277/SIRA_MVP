...
function ResultsScreen({ origin, destination, journeys, selectedId, onSelect, onBack }: {
  origin: Place; destination: Place; journeys: Journey[]; selectedId: string; onSelect: (journey: Journey) => void; onBack: () => void;
}) {
  const ordered = [...journeys].sort((a, b) => a.recommended ? -1 : b.recommended ? 1 : a.duration - b.duration);
  return <section className="app-screen content-screen"><ScreenHeader title="Trajets proposés" onBack={onBack} /><div className="route-summary"><span><Crosshair /><b>Départ</b><em>{origin.name}</em></span><span><MapPin /><b>Destination</b><em>{destination.name}</em></span></div><div className="results-map"><SiraMap origin={origin} destination={destination} journeys={journeys} selectedJourneyId={selectedId} /></div><p className="data-banner"><Route size={15} />SIRA-MORE · Pareto → diversité → score explicable · 325 lignes Grand Abidjan</p><div className="results-list">{ordered.map((journey) => <ResultCard key={journey.id} journey={journey} selected={journey.id === selectedId} onSelect={() => onSelect(journey)} />)}</div></section>;
}
...
export default function SiraApp() {
  ...
  const calculate = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/mobility/journeys`, { method: "POST", headers: { "content-type": "application/json" }, signal: AbortSignal.timeout(30_000), body: JSON.stringify({ origin: { lat: origin.coordinates[1], lon: origin.coordinates[0], name: origin.name }, destination: { lat: destination.coordinates[1], lon: destination.coordinates[0], name: destination.name } }) });
      if (!response.ok) {
